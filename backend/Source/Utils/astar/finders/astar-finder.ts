...
export class AStarFinder {
    ...
    constructor({
        ...
        this.grid = new Grid({
            width: grid.width,
            height: grid.height,
            matrix: grid.matrix || undefined,
            densityOfObstacles: grid.densityOfObstacles || 0
        });
    ...
    public findPath(startPosition: IPoint, endPosition: IPoint): number[][] {
        ...
        if (
            !this.isValidPosition(endPosition) ||
            !this.isValidPosition(startPosition) ||
    ...
    private isValidPosition(position: IPoint): boolean {
        ...
        if (Math.floor(position.x) != position.x || Math.floor(position.y) != position.y)
            return false;
        return true;
    }
}
